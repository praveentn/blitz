// =================== src/pages/PromptsPage.js ===================
import React, { useState, useEffect } from 'react';
import { PlusIcon, DocumentTextIcon } from '@heroicons/react/24/outline';
import { apiService, handleApiError } from '../services/api';
import LoadingSpinner, { TableSkeleton } from '../components/LoadingSpinner';

const PromptsPage = () => {
  const [prompts, setPrompts] = useState([]);
  const [loading, setLoading] = useState(true);

  useEffect(() => {
    loadPrompts();
  }, []);

  const loadPrompts = async () => {
    try {
      const response = await apiService.prompts.getAll();
      setPrompts(response.data);
    } catch (error) {
      handleApiError(error, 'Failed to load prompts');
    } finally {
      setLoading(false);
    }
  };

  if (loading) {
    return (
      <div className="p-6">
        <div className="mb-6">
          <h1 className="text-2xl font-bold text-gray-900">Prompts</h1>
          <p className="text-gray-600">Manage your prompt templates</p>
        </div>
        <TableSkeleton />
      </div>
    );
  }

  return (
    <div className="p-6">
      <div className="mb-6">
        <div className="flex justify-between items-center">
          <div>
            <h1 className="text-2xl font-bold text-gray-900">Prompts</h1>
            <p className="text-gray-600">Manage your prompt templates</p>
          </div>
          <button className="btn-primary">
            <PlusIcon className="h-5 w-5 mr-2" />
            Add Prompt
          </button>
        </div>
      </div>

      <div className="card">
        <div className="card-header">
          <h3 className="text-lg font-medium">Prompt Templates</h3>
        </div>
        <div className="card-body">
          {prompts.length === 0 ? (
            <div className="text-center py-12">
              <DocumentTextIcon className="mx-auto h-12 w-12 text-gray-400" />
              <h3 className="mt-2 text-sm font-medium text-gray-900">No prompts</h3>
              <p className="mt-1 text-sm text-gray-500">Get started by creating a new prompt template.</p>
            </div>
          ) : (
            <div className="overflow-x-auto">
              <table className="table">
                <thead className="table-header">
                  <tr>
                    <th className="table-header-cell">Name</th>
                    <th className="table-header-cell">Category</th>
                    <th className="table-header-cell">Version</th>
                    <th className="table-header-cell">Created</th>
                    <th className="table-header-cell">Actions</th>
                  </tr>
                </thead>
                <tbody className="table-body">
                  {prompts.map((prompt) => (
                    <tr key={prompt.id} className="table-row">
                      <td className="table-cell font-medium">{prompt.name}</td>
                      <td className="table-cell">{prompt.category || 'General'}</td>
                      <td className="table-cell">{prompt.version}</td>
                      <td className="table-cell">{new Date(prompt.created_at).toLocaleDateString()}</td>
                      <td className="table-cell">
                        <button className="btn-secondary">Edit</button>
                      </td>
                    </tr>
                  ))}
                </tbody>
              </table>
            </div>
          )}
        </div>
      </div>
    </div>
  );
};

export { PromptsPage };
export default PromptsPage;